...
const WorkingHoursController = {
  getWorkingHours: async (req, res) => {
    try {
      ...
      const eventIDs = eventsResponse.responseBody.map(row => row.EventID);
      ...
      const result = students.map(student => {
        ...
        for (const eventID of eventIDs) {
          ...
          const totalHours = attHours > 0 ? attHours + coordHours : coordHours;
      ...
      return res.status(200).json({
        columns: ['RollNo', 'Name', ...eventIDs.map(id => eventIDToName.get(id))],
        data: result
      });
